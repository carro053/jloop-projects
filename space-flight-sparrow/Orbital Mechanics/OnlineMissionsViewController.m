...
- (void) viewMissionId:(NSTimer *)timer
{
    ...
    [self presentViewController:viewOnlineMissionViewController animated:YES completion:nil];
}
...
{
	id response = [self objectWithUrl:[NSURL URLWithString:[NSString stringWithFormat:@"http://gravity.jloop.com/puzzles/getTotalMissions/35"]]];
...
{
	id response = [self objectWithUrl:[NSURL URLWithString:[NSString stringWithFormat:@"http://gravity.jloop.com/puzzles/getPuzzles/%d/%d/%d/35",orderByRating,missionOffset,missionsPerPage]]];
	NSArray *feed = (NSArray *)response;
	return feed;
}
