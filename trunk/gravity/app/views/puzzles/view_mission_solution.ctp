		...
		function imageLoaded(img,pos_x,pos_y) {
		    ...
		    */
		    ...
		    for (y = 0; y < height; y++) {
		        for (x = 0; x < width; x++) {
		        	var index = y * width * 4 + x * 4;
		            imageData.data[index + 3] *= Math.min(1,(765 - imageData.data[index + 0] - imageData.data[index + 2] - imageData.data[index + 3]) / 765 * 0.6 + 0.3);
		        }
		    }
		    ctx.clearRect(0, 0,width, height);
			ctx.putImageData(imageData, 0, 0);
			contextScene.beginPath();
			contextScene.arc(pos_x+141,pos_y+141,160,0,2 * Math.PI,false);
