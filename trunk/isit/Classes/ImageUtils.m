...
Image *createImage(uint8_t *bytes, size_t bprow, int srcWidth, int srcHeight, int theCM) {
	// create the image
	...
	if(theCM == 0)
	{
		...
		for(int y=0; y<srcHeight; y++) {
			...
			for(int x=0;x<srcWidth;x++) {
				...
				srcRowPtr+=2;			// skip past the alpha value
				// convert to greyscale
				result->pixels[x][y] = MAX(0,MIN(100,MIN(1.0*red-1.0*green,1.0*red-1.0*blue)));
			}
		}
	}else if (theCM == 2) {
		for(int y=0; y<srcHeight; y++) {
			...
			for(int x=0;x<srcWidth;x++) {
				...
				srcRowPtr+=2;			// skip past the alpha value
				// convert to greyscale
				result->pixels[x][y] = MAX(0,MIN(100,2.0*green-1.0*red-1.0*blue));
			}
		}
	}else if (theCM == 3) {
		for(int y=0; y<srcHeight; y++) {
			...
			for(int x=0;x<srcWidth;x++) {
				...
				srcRowPtr+=2;			// skip past the alpha value
				// convert to greyscale
				result->pixels[x][y] = MAX(0,MIN(100,2.0*blue-1.0*green-1.0*red));
			}
		}
	}else if (theCM == 4) {
		for(int y=0; y<srcHeight; y++) {
