...
class QuestionsController extends AppController {
	...
	function mexport($game_id)
	{
		...
		foreach($questions as $question):
			...
			$request = new RestRequest('http://admin:MyAdminPass87@50.56.194.198:8282/RingorangWebService/rservice/game/createQuestion', 'POST',json_encode($data));
			$request->execute();
			$response = $request->getResponseBody();
			pr($request);
...
',htmlspecialchars_decode($question['Question']['answer_4_note'], ENT_QUOTES));
	...
	function export()
	{
		...
		$request = new RestRequest('http://admin:MyAdminPass87@50.56.194.198:8282/RingorangWebService/rservice/game/getList?appId=22&offset=0&count=10', 'POST', $data);
		$request->execute();
		$response = $request->getResponseBody();
		echo $response;
		die;
