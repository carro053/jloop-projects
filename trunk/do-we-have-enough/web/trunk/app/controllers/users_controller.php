...
class UsersController extends AppController
{
	...
	function event_list()
	{
		...
		$this->set('data',$this->Event->findAll(null,null,'Event.created DESC',30));
	}
	...
	function check_email()
	{
		...
		if(isset($user['User']['id']))
		{
			...
			{
				$user['Group'][$key]['User'] = $this->User->findAll('User.id IN(SELECT `user_id` FROM `groups_users` WHERE `group_id` = '.$group['id'].') AND User.id != '.$user['User']['id']);
			}
...
If you wish to unsubscribe from dowehaveenough.com - http://".$this->environment.".dowehaveenough.com/unsubscribe/".$user['email']."/".$user['EventsUser']['hash'];
	...
	function current_events()
	{
		...
		$this->set('meta_description','Do You Have Enough? Do We Have Enough? Need to find out if you have enough players for a ball gameɠEnough seats to fill a poker tableɠEnough participants to hold today\'s meeting?');
