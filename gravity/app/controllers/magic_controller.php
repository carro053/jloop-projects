...
class MagicController extends AppController {
	...
	function deck_create()
	{
		if(isset($this->data['Deck']['name']))
		{
			...
			$this->redirect('/magic/deck_edit/'.$this->Deck->id);
		}else{
	...
	function deck_manage($deck_id)
	{
		...
		$this->set('deck',$deck);
		$this->set('cards',$this->Card->find('all',array('order'=>'Card.color ASC')));
	}
	...
	function deck_add_card($deck_id,$card_id)
	{
		...
		echo $this->DeckCard->save($deck_card);
		exit();
	}
	...
	function deck_remove_card($deck_id,$card_id)
	{
		...
		{
			echo $this->DeckCard->delete($deck_card['DeckCard']['id']);
		}else{
			echo 0;
		}
	}
