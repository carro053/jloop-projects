 ...
 */
	function initialize(&$controller, $settings = array()) {
		...
		$this->delivery = 'postmark';
		$this->from = '<events@dowehaveenough.com>';
	}
 ...
 */
	function __postmark() {
		...
		// To
		$message['From'] = $this->__formatAddress($this->from);
		if (is_array($this->to)) {
			$message['To'] = implode(', ', array_map(array($this, '_formatAddress'), $this->to));
		} else {
			$message['To'] = $this->__formatAddress($this->to);
		}
		...
		if (!empty($this->cc)) {
			if (is_array($this->cc)) {
				...
				$message['Cc'] = $this->__formatAddress($this->cc);
			}
		}
		...
		if (!empty($this->bcc)) {
			if (is_array($this->bcc)) {
				...
				$message['Bcc'] = $this->__formatAddress($this->bcc);
			}
		}

		// Subject
		$message['Subject'] = $this->__encode($this->subject);
		...
		if (!empty($this->replyTo)) {
			$message['ReplyTo'] = $this->__formatAddress($this->replyTo);
		}
