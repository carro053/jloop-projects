						...
						<tr id="groups_lead_id_<?php echo $lead['GroupsLead']['id']; ?>">
							...
							<td>
								...
								<?php if(empty($lead['highrise_id'])) { ?>
									<a href="#" onclick="exportToHighrise(<?php echo $lead['id']; ?>); return false;" class="highrise-export-<?php echo $lead['id']; ?>">Export to Highrise</a>
								<?php } else { ?>
									<a href="https://jloop.highrisehq.com/companies/<?php echo $lead['highrise_id']; ?>" target="_blank" class="highrise-export-<?php echo $lead['id']; ?>">View in Highrise</a>
								<?php } ?>&nbsp;
